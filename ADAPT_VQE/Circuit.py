...
class Circuits_Composser():
    ...
    def Observable_index_to_Pauli(self, index):
        ...
        fermion_op = FermionOperator(f"{i}^ {j}^ {k} {l}", 1)  # i a†_i a†_j a_k a_l
        fermion_op += FermionOperator(f"{k}^ {l}^ {i} {j}", 1)  # i a†_k a†_l a_i a_j
    ...
    def Qibo_measure_energy_no_diagonalization_4_index(self, circuits=None):
        ...
        for i in range(1, len(Qibo_circs)):
            ...
            if len(Qibo_circs[i]) == 4: #the data structure for the 4 diferent index just has 3 parameters
                ...
                for bitstring in bitstrings:
                    ...
                    E_ijkl += sign * op_amplitude * (res.probabilities(afected)[int(bitstring, 2)]) 
        ...
        for index in E_index:
            if abs(E_index[index][0])>10**(-8):
                print(np.round(E_index[index][0],8), '\t\t', list(index), '\t\t', np.round(E_index[index][1],8))
        ...
        for op in self.operator_pool:
            ...
            if len(set(indexes)) == 4:
                ...
                for term, coef in observable.terms.items():
                    ...
                    circuits.append([circuit, -matrix_element*coef.real, obs, indexes])
